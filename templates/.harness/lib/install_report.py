...
COMPONENTS = [
    ...
    ("Design-system gate (ds-gate ratchet + colour-pair check)", "use_ds_gate",
     ...
     False),
    ("hookify rules + declared plugin engine", "use_hookify",
     "Declarative guards in .claude/hookify.*.local.md, plus the engine they need declared as a project dependency in .claude/settings.json (extraKnownMarketplaces + enabledPlugins).",
     "The rules are inert markdown without the hookify plugin; the declaration makes Claude Code ask you to install it when you trust this folder. The installer never writes to ~/.claude — declaring is the project's job, installing is yours.",
     True),
    ("Context tooling decision graph (blast radius)", "use_context_graph",
     "Deterministic routing between code graph, grep and LSP before changing a shared symbol, plus the hookify guards that fire on core-path and database-write edits.",
     "The routing is installed as rules + prose; the graph CLI itself is an EXTERNAL dependency the installer never installs (it lives outside the target root). Without harness_core_paths the P1 guard is not generated.",
     False),
