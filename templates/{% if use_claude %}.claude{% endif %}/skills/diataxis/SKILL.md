...
> **Provenance:** This skill is an adaptation of the Diátaxis framework by
> Daniele Procida (https://diataxis.fr/, CC BY-SA 4.0). Canonical content is
> marked with "Source:" links pointing to the official page. The curated
> distillation shipped with this framework lives in [`references/`](./references/)
> (12 files) — the original curator's full 17-page verbatim snapshot,
> `PROVENANCE.json` index and SHA-256 hashes are NOT part of this repo (they
> live only in the private working set of the person who first curated this
> skill). Pragmatic extensions (templates, long worked examples, checklists,
...
This skill helps you create high-quality, user-focused documentation following the Diataxis framework, which organizes documentation into four distinct types based on user needs. https://diataxis.fr/
...
**For Quality theory (functional vs deep):**
Load [Quality Reference](./references/quality.md) when you need to:
- Distinguish what Diátaxis can and cannot do for your docs
- Argue about quality with stakeholders
- Audit existing documentation against canonical quality criteria

**For Foundations (why exactly four types):**
Load [Foundations Reference](./references/foundations.md) when you need to:
- Explain to skeptics why the framework has 4 types (not 3 or 5)
- Understand the 2D model deeply
- Map any documentation artifact to a quadrant

**For the Compass:**
Load [Compass Reference](./references/compass.md) when you need to:
- Classify ambiguous content
- Apply the framework at sentence/paragraph level

**For Complex Hierarchies:**
Load [Complex Hierarchies Reference](./references/complex-hierarchies.md) when:
- Documentation spans multiple products / audiences / platforms
- You need to handle landing pages and lists of >7 items
- You face a 2-dimensional organisational problem (Diátaxis × topic / user-type / deployment)

**For Tutorial-vs-How-to distinction:**
Load [Tutorials vs How-to Reference](./references/tutorials-vs-howto.md) when:
- You suspect a doc conflates the two (the most common confusion)
- You need the 15-point contrast table

**For Reference-vs-Explanation distinction:**
Load [Reference vs Explanation Reference](./references/reference-vs-explanation.md) when:
- Reference material is starting to drift into explanatory narrative
- Explanation is starting to become an inventory of facts

**For Canonical Language Patterns:**
Load [Canonical Language Reference](./references/canonical-language.md) when:
- Writing in the actual voice prescribed by Diátaxis
- Reviewing a doc for canonical tone
