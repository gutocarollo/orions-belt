...
## Canonical principles (from diataxis.fr/tutorials/)

The official Diátaxis page enumerates the following anti-patterns and rules.
The skill expands them with concrete examples below, but the canonical statements
should be respected as written:

- **Don't try to teach.** Provide a learning experience; do not lecture. The
  teacher's anxiety to impart knowledge is the primary trap.
- **Obligations of the teacher.** The exercise must be: _meaningful_,
  _successful_, _logical_, _usefully complete_.
- **You are required to be present, but condemned to be absent.** Documentation
  cannot rescue a learner the way a teacher in the room can — therefore the
  tutorial must be so reliable that things _cannot_ go wrong.
- **Target the feeling of doing.** Skill becomes pleasure through purpose,
  action, thinking and result joined up.
- **Encourage and permit repetition.** Repetition reaffirms success; design
  steps so they can be repeated.
- **Point out what the learner should notice.** Close the loops of learning
  by drawing attention to environmental signs (e.g., how a prompt changes).
- **Ruthlessly minimise explanation.** Link out, do not embed.
- **Ignore options and alternatives.** Make decisions for the learner.
- **Aspire to perfect reliability.** Every step works for every user every time.

Source: https://diataxis.fr/tutorials/#key-principles
...
### Canonical Language Patterns (Verbatim from Diátaxis)

Tutorials in Diátaxis use a deliberate mix of imperatives, first-person plural,
and orientation cues. Use these patterns; do not abolish "we" or "let's" —
imperatives and first-person plural **coexist** in a Diátaxis tutorial.
...
| `We must always do x before we do y because… (see Explanation for details).` | Minimal in-line explanation, links out for depth. |
...
Do not mark "Let's check" or "We can run" as bad — they are canonical.
...
> **Note on provenance:** The 3 patterns below (Installation / Build / Hands-On)
> and the long worked example further down are this skill's own pragmatic
> distillation, **not part of canonical Diátaxis**. They are useful templates,
> but treat the canonical analogy ("teaching a child to cook") and the official
> key principles as authoritative when in tension.
...
## Applied to food and cooking (canonical analogy)

Teaching a child to cook is the canonical analogy for a tutorial. From
diataxis.fr/tutorials/#applied-to-food-and-cooking:
...
- Success is when the child **acquires the knowledge and skills** you hoped to
  impart — not the culinary outcome.
- The child must **discover pleasure** in the experience and want to return.
  Learning a skill always requires repetition.
- The lesson is framed around making a particular dish, but what the child
  actually needs to learn is: that we wash our hands, how to hold a knife, why
  the oil must be hot, what this utensil is called, how to time and measure.
- The child learns through the **activities** done together, not from what is
  said or shown.
- With a young child, the lesson may have to end before completion. That is
  expected. As long as the child achieved something and enjoyed it, foundation
  has been laid.

Hold this analogy in mind when writing software tutorials. The "dish" is the
artifact; the "skills" are the real curriculum.
