...
## Reference as description (canonical framing)
...
about the territory, without having to go out and check the territory for
yourself; a reference guide serves the same purpose for the product and its
internal machinery.
...
Source: https://diataxis.fr/reference/#reference-as-description

## Core Principles

### 1. Describe, Don't Instruct

Reference describes what something is or does. It should not attempt to show
how to perform tasks (those belong in how-to guides and tutorials), **but it
can and often needs to include a description of how something works or the
...
From diataxis.fr/reference/#the-language-of-reference-guides:

| Pattern | Function |
|---|---|
| `Django's default logging configuration inherits Python's defaults. It's available as django.utils.log.DEFAULT_LOGGING…` | State facts about the machinery. |
| `Sub-commands are: a, b, c, d, e, f.` | List commands, options, operations, features, flags, limitations, errors. |
...
Food packaging labels are the canonical analogy for reference. From
diataxis.fr/reference/#applied-to-food-and-cooking:

- When looking for information, you do not want opinions, speculation,
  instructions or interpretation.
- Information is presented in **standard ways** so you can find it quickly:
  nutritional properties, storage, ingredients, health implications.
- Standard phrases: _May contain traces of wheat._ _Net weight: 1000g._
- You will **certainly not expect** recipes or marketing claims mixed in.
  That could be **literally dangerous**.
- Reference material on food products is so important that it is usually
  **governed by law**. The same kind of seriousness should apply to all
