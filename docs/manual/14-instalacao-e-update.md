...
Pré-requisito bloqueante: `python3` deve resolver para Python ≥3.14. No macOS, instale
`python@3.14` via Homebrew e mantenha o bin do Homebrew antes de `/usr/bin` no `PATH`; o Python
3.9 do Xcode pode permanecer instalado, mas não é um runtime suportado pelo instalador.
...
2. **Planeja sem mutar:** [`.harness/lib/install_apply.py`](../../templates/.harness/lib/install_apply.py) fixa a raiz POSIX por descritor (`O_DIRECTORY|O_NOFOLLOW`), enumera o render e calcula todos os merges usando operações `*at()` relativas ao descritor — sem depender de `/proc` ou `/dev/fd`. Symlink, troca da raiz, escape, colisão desconhecida, arquivo owned alterado localmente ou edição entre plano e apply abortam.
...
- **`.harness/INSTALL-REPORT.md`** — o **report de decisão** (auditoria da instalação), regenerado a cada (re)install por [`.harness/lib/install_report.py`](../../templates/.harness/lib/install_report.py). Lista o que foi instalado E o que NÃO foi, a flag que gateou cada componente, a razão e um **exemplo aplicado** ao repo real (ex.: "UI-evidence pulado — `use_ui_evidence=false`: editar `<web>/app/*.tsx` não exige PNG"), as decisões file-level por estratégia (owned/marked-block/structured-json/preserve com os paths reais) e a decisão de hook-manager. Derivado só do manifest + answers — nada inventado. É parte do processo, não um extra opcional.
