...
Um **hook**, no vocabulário deste framework, é um alarme automático ligado a um evento do ciclo de vida do agente: quando o evento acontece, o alarme dispara sozinho um script, sem que ninguém precise lembrar de rodar nada manualmente. O evento aqui é o `SessionStart` — o instante em que uma sessão nasce (ou uma sessão antiga é retomada, ou o contexto acabou de ser compactado). Um hook pode declarar um **matcher**: um filtro que restringe em quais variantes do evento ele dispara. `"matcher": "compact|resume"` faria um hook tocar SÓ quando a sessão reabre depois de compactação ou resume, nunca numa sessão nova comum — nenhum dos hooks abaixo usa esse filtro hoje (o `marathon-reinject` usava, e a seção dele explica por que deixou de usar).
...
Duas exceções, quando a run está **pausada** (capítulo 08): com a janela válida, o hook injeta `status="paused"` e **omite o checklist** — injetar o checklist é o que faz um modelo retomar sozinho; com a janela expirada, injeta `status="pause-expired"` e o estado com instrução de PERGUNTAR antes de qualquer trabalho, nunca de executar.
