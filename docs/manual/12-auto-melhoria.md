...
O cap de linhas é o orçamento de contexto: quando o arquivo cresce além dele, as lições antigas saem da janela de injeção — e é isso que força o passo seguinte. **O corte é ANUNCIADO, nunca silencioso** (relato de adopter, 2026-07-30): passando do teto, o hook imprime um `WARNING` dizendo quantas linhas ficaram fora, que são as do **INÍCIO** do arquivo — as lições mais ANTIGAS — e oferece as duas saídas na ordem certa: promover lição antiga a regra permanente (preferido) ou subir `HARNESS_LESSONS_INJECT_MAX_LINES`. O caso que motivou o aviso: um adopter com 84 linhas contra teto 80 já perdia o cabeçalho do arquivo em toda sessão e perderia a lição nº 1 no próximo append, enquanto a instrução escrita dizia que o corte era no fim — protegendo o lado errado do arquivo. Truncar sem dizer produz exatamente a classe de defeito que este harness existe para evitar: um sinal que o agente não vê.
