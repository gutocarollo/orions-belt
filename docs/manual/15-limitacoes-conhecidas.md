...
- **Confinamento portátil do instalador:** `install_apply.py` ancora a raiz em um descritor aberto e faz traversal, leitura, backup, escrita atômica, rollback e limpeza com a família POSIX `*at()` exposta por `dir_fd`, sempre abrindo ancestrais com `O_NOFOLLOW`. Não depende de `/proc/self/fd` (Linux) nem de traversal sob `/dev/fd` (inexistente no macOS). A regressão cobre tanto a troca do path da raiz quanto a troca de um diretório intermediário por symlink; em ambos os casos nenhum arquivo externo pode ser escrito.
