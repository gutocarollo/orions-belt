...
**O que faz** — Sem binding positivo para `runtime:session_id`: inerte, mesmo que existam outras runs no registry ou um `ACTIVE` legado. Com maratona vinculada: resolve o `run_id`, valida o índice e, se o checklist tem itens abertos (`- [ ]`), bloqueia devolvendo a "Próxima ação" registrada. Paradas legítimas: checklist zerado, ou a seção "Próxima ação" começando com `AGUARDANDO:` (bloqueado em decisão do usuário). Com `stop_policy: soft`, `HARNESS_MARATHON_MAX_BLOCKS_WITHOUT_PROGRESS` bloqueios consecutivos sem mudança de conteúdo liberam com aviso. Com `stop_policy: hard`, usado para condições terminais explícitas, o escape é desabilitado enquanto houver trabalho executável. A biblioteca `marathon-locate.sh` é dependência obrigatória: ausência ou source inválido falha fechado.

**Exemplo (cenário simulado)** — Numa maratona ativa no projeto `demo-app`, o checklist do `RUN.md` ainda tem 2 itens abertos e a "Próxima ação" não começa com `AGUARDANDO:`. O agente tenta encerrar o turno assim mesmo; o gate bloqueia e devolve a "Próxima ação" registrada. Se a run for `soft` e isso se repetir por `HARNESS_MARATHON_MAX_BLOCKS_WITHOUT_PROGRESS` vezes sem o checksum do `RUN.md` mudar, o gate libera com aviso. Se for `hard`, continua bloqueando; para interromper é necessário pausar, registrar `AGUARDANDO:` ou encerrar explicitamente.
...
**O que faz** — Delega ao modo `reap` do [dev-doctor genérico](../../templates/.harness/hooks/dev-doctor.sh), mas nunca descobre nem mata processos globalmente. Um launcher que deseja limpeza automática registra `<pid> <start_token> <reap_after_epoch>` em `<HARNESS_PID_REGISTRY_DIR>/<nome>.pid`; o token numérico vem de `dev-doctor.sh pid-token <pid>` e usa os start ticks de `/proc` no Linux (maior resolução) ou o start-time de `ps` no macOS. Antes de enviar `TERM`, o reaper prova simultaneamente: registry contido no projeto, entrada regular não-symlink, lease expirada, PID ainda existente, token de start-time igual (proteção contra reúso de PID), cwd vivo dentro da raiz e comando reconhecido como tooling de agente/headless. O cwd/comando usam `/proc` quando disponível e `lsof`/`ps` no macOS. Registro prova ownership; só a expiração explícita prova autorização de cleanup. Entrada inválida, lease ativa, processo externo ou comando não reconhecido gera apenas WARN. Processo de alta CPU e vida longa também gera só WARN.
...
Os porteiros do Stop são a materialização de "definition of done" como código: prova antes de claim, pixel antes de "UI pronta", checklist antes de parar, faxina antes de sair. Cada um tem escape hatch honesto (reformular o veredito; SKIP com TTL e motivo; `AGUARDANDO:`; `soft` com anti-prisão ou `hard` com pausa/encerramento explícitos) — gate sem válvula legítima ensina o agente a contornar o gate.
