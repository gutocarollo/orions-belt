...
`tomllib` is stdlib only from 3.11 on. The small parser below supports quoted
values and treats `#` as a comment delimiter only at the start of a value or
after whitespace. This keeps ordinary values such as `C# Academy` and
`p@ss#word` intact without adding a dependency.
...
(a direct `source` of the `.conf` would execute shell substitutions and would
silently diverge from the quote-aware parser and multi-name file fallback) — exactly the kind
